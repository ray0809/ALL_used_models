...
def GetSampling(imgFile, maskFile):
    ...
    random_nb = random.sample(range(len(imgs)), int(len(imgs)*0.9))
    train_samples = []
    val_samples = []
    for i in range(len(imgs)):
        imgPath = os.path.join(imgFile,imgs[i])
...
class DataLoader(Sequence):
    def __init__(self, samples, class_nb, transformer, batch_size=16, shuffle=True):

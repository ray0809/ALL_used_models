...
def crtModel(HEIGHT, WIDTH, CLASSNB, activation='sigmoid', backbone='xception'):
    inputs = Input(shape=(HEIGHT, WIDTH, 3))
    x = Lambda(preprocess_input)(inputs)
    base_model = Deeplabv3(weights='pascal_voc', classes=CLASSNB)
    x = base_model(x)
    x = Activation(activation=activation)(x)
    return Model(inputs,x)

if __name__ == '__main__':
    base_model = Deeplabv3(weights=None, classes=1)
    base_model.summary()
...
class Eval:
    ...
    def addBatch(self, predict, gth):
        predict = np.squeeze(np.argmax(predict, axis=-1))
        gth = np.squeeze(np.argmax(gth, axis=-1))
        ...
        mIou = np.nanmean(per_class_iu[1:]) 
    ...
    def getMetric(self):
        ...
        return overall_acc, per_class_acc, per_class_iu, mIOU  # 剔除背景
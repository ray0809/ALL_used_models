...
def GetSampling(main_path):
    ...
    for root, dirs, files in os.walk(main_path):
        if len(files) > 90:
            ...
            samples.append([os.path.join(root,f) for idx,f in enumerate(files) if 'jpg' in f and idx < 120])
    return samples
...
class dataLoader(Sequence):
    def __init__(self, samples, batch_size, shuffle=False, transformer=None):
